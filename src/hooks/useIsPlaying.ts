import AudioPro from '..';
import { State } from '../constants';
import { usePlayWhenReady } from './usePlayWhenReady';
import { usePlaybackState } from './usePlaybackState';

/**
 * Tells whether the AudioPro is in a mode that most people would describe
...
/**
 * This exists if you need realtime status on whether the AudioPro is
...
export async function isPlaying() {
  const [playbackState, playWhenReady] = await Promise.all([
    AudioPro.getPlaybackState(),
    AudioPro.getPlayWhenReady(),
  ]);
